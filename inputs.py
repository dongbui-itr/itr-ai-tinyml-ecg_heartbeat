...
def _process_sample_freeze(use_gpu_index,
                           lock,
                           file_path,
                           buf_em,
                           buf_ma,
...
def _process_files_batch_freeze(use_gpu_index,
                                queue,
                                lock,
                                process_index,
                                ranges,
    ...
    for s in range(num_shards_per_batch):
        ...
        for i in files_in_shard:
            ...
            try:
                ...
                res_db_dict = _process_sample_freeze(use_gpu_index=use_gpu_index,
                                                     lock=lock,
                                                     file_path=file_name,
                                                     buf_em=buf_em,
                                                     buf_ma=buf_ma,
...
def build_tfrecord_freeze(use_gpu_index,
                          db_process_info,
                          total_shards,
                          datastore_dict,
                          all_file,
    ...
    for process_index in range(len(ranges)):
        ...
        t = Process(target=_process_files_batch_freeze, args=args)
        t.start()
        processors.append(t)
    ...
    for label in datastore_dict["beat_class"].keys():
        if not 'NOTABEAT' in label:
            ...
            for studyID in list_data['Train'][f'{label}_study_path']:
                ...
                if len(list_files) == 0:
                    list_files = [i_file.replace(f'.{EXT_BEAT}', '') for i_file in
                                  glob(studyID + '/*/*.{}'.format(EXT_BEAT))]
                if len(list_files) > 0:
...
def create_tfrecord_portal_transfer(data_model_dir,
                                    data_dir,
                                    # media_dir,
                                    save_image=False,
                                    org_num_processes=os.cpu_count(),
    ...
    if not os.path.exists(data_model_dir):
        os.makedirs(data_model_dir)
    elif not over_write and os.path.exists(data_model_dir + 'datastore_transfer.txt'):
        print("{} exist!".format(data_model_dir + 'datastore_transfer.txt'))
        return
    elif over_write:
        shutil.rmtree(data_model_dir)
        os.makedirs(data_model_dir)
...
def create_tfrecord_from_portal_event2_retrain(data_model_dir,
                                               data_dir,
                                               media_dir,
                                               save_image=False,
                                               org_num_processes=os.cpu_count(),
    ...
    a = 10
    all_file_train = []
    with open(data_model_dir + 'ds_train.txt', 'r') as fp:
        fp.readline()
        i = 0
        while True:
            line = fp.readline()
            try:
                all_file_train.append(line.split(',')[1])
            except:
                break
    ...
    with open(data_model_dir + 'ds_eval.txt', 'r') as fp:
        fp.readline()
        while True:
            line = fp.readline()
            try:
                all_file_eval.append(line.split(',')[1])
            except:
                break
    ...
    for t in ["train", "eval"]:
        ...
        try:
            ...
            datastore_dict = build_tfrecord_freeze(use_gpu_index=0,
                                                   db_process_info=db_process_info,
                                                   total_shards=total_shards,
                                                   datastore_dict=datastore_dict,
                                                   all_file=all_file,
...
def main(argv=None):
    MEDIA_PATH = '/media/{}/Data_4T1/AI-Database/Research/QRS_classification/TinyML-HeartBeat/240529_NSV_bk'.format(
        getpass.getuser())
    DATA_PATH = '/media/{}/Data_4T1/AI-Database/Research/5s_data/Collection_20240510/'.format(getpass.getuser())
    # sampling_rate
    # feature_len
    # num_block
    # bwr
    ...
    for d in datasets:
        ...
        create_tfrecord_portal_transfer(data_model_dir=data_model_dir,
                                        data_dir=DATA_PATH,
