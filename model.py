...
def conv1d_net_squeeze_2(x,
    ...
    if x.shape[-1] <= num_filters:
        x = conv_squeeze(x, squeeze=8, expand=num_filters//2, strides=1, name=name + '_conv1d_squeeze')
...
def conv_squeeze(_x,
    ...
    if expand >= 16:
        right_left = keras.layers.Conv1D(expand // 2, 1, strides=strides, activation=conv_activation, padding='same',
                                    name=f_name.format(name, "expand3_l"))(x)
        right_right = keras.layers.Conv1D(expand // 2, 3, strides=strides, activation=conv_activation, padding='same',
                                         name=f_name.format(name, "expand3_r"))(x)
        right = keras.layers.concatenate([right_left, right_right], axis=2, name=f_name.format(name, "concat_2"))
    else:
...
def beat_concat_seq2_add_more2_128Hz(feature_len,
                                    num_of_class=2,
                                    from_logits=False,
                                    filters_rhythm_net=None,
                                    num_loop=3,
    ...
    for st, ff in enumerate(filters_rhythm_net):
        ...
        x = conv1d_net(x=x,
                       ...
                       name="resnet12" + name)
        # Batch norm, Activation, Dropout, Convolution (stride=1)
        if 'resnet11b_2_stage_3' in "resnet11" + name:# in 'resnet11b_2_stage_3':
            a=10

        x = conv1d_net_squeeze_2(x=x,
                               num_filters=f2,
                               kernel_size=3,
                               strides=1,
                               pad='SAME',
...
def beat_concat_seq2_add_more2_250Hz(feature_len,
    ...
    input_layer = keras.layers.Input(shape=(feature_len,))
    resnet_input_layer = keras.layers.Reshape((feature_len + 6, 1))(input_layer)
    # Convolution(stride=2)
    x = conv1d_net(x=resnet_input_layer,
                   num_filters=filters_rhythm_net[0][0],
                   kernel_size=4,
