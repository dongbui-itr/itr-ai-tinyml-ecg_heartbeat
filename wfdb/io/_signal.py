        ...
        if expanded:
            ...
            for ch in range(self.n_sig):
                if len(self.e_d_signal[ch]) != spf[ch] * self.sig_len:
                    raise ValueError(
                        'Length of channel ' + str(ch) + 'does not match samps_per_frame[' + str(ch + ']*sig_len'))
            ...
            for ch in range(self.n_sig):
                ...
                if (chmin < dmin) or (chmax > dmax):
                    outside_pos = np.where(self.d_signal[:, ch] > dmax)[0]
                    self.d_signal[:, ch][outside_pos] = dmax
                    outside_pos = np.where(self.d_signal[:, ch] < dmin)[0]
                    self.d_signal[:, ch][outside_pos] = dmin
                    print("check_sig_cohesion - Channel " + str(ch) + " contain values outside allowed range [" + str(
                        dmin) + ", " + str(
                        dmax) + "] for fmt " + str(fmt))

                    # raise IndexError(
                    #     "Channel " + str(ch) + " contain values outside allowed range [" + str(dmin) + ", " + str(
                    #         dmax) + "] for fmt " + str(fmt))
            ...
            if expanded:
                for ch in range(self.n_sig):
                    ...
                    np.multiply(self.e_p_signal[ch], self.adc_gain[ch],
                                self.e_p_signal[ch])
                    np.add(self.e_p_signal[ch], self.baseline[ch],
                           self.e_p_signal[ch])
    ...
    if smooth_frames or sum(samps_per_frame) == n_sig:
        ...
        for fn in w_file_name:
            signals[:, out_dat_channel[fn]] = _rd_dat_signals(fn, dir_name, pb_dir,
                                                              w_fmt[fn], len(datchannel[fn]), sig_len,
                                                              w_byte_offset[fn],
    ...
    # Extra frames present to be smoothed. Obtain averaged uniform numpy array
        ...
        for ch in range(n_sig):
            if samps_per_frame[ch] == 1:
                ...
                signal[:, ch] = [np.average(sig_data[ind:ind + samps_per_frame[ch]]) for ind in
                                 range(startind, len(sig_data), tsamps_per_frame)]
        # Skew the signal
        signal = _skew_sig(signal, skew, n_sig, read_len, fmt, nan_replace)
    ...
    # expanded samples.
        ...
        for ch in range(n_sig):
            ...
            ch_indices = np.concatenate([np.array(range(samps_per_frame[ch]))
                                         ...
                                         + tsamps_per_frame * framenum for framenum in
                                         range(int(len(sig_data) / tsamps_per_frame))])
...
def _blocks_to_samples(sig_data, n_samp, fmt):
    ...
    if fmt == '212':
        ...
        sig[0::3] = (sig_data[0::4] >> 1)[0:len(sig[0::3])] + 128 * np.bitwise_and(sig_data[1::4], 0x07)[
                                                                    0:len(sig[0::3])]
        # Second signal is 7 msb of third byte and 3 lsb of forth byte
        sig[1::3] = (sig_data[2::4] >> 1)[0:len(sig[1::3])] + 128 * np.bitwise_and(sig_data[3::4], 0x07)[
                                                                    0:len(sig[1::3])]
        # Third signal is 5 msb of second byte and 5 msb of forth byte
        sig[2::3] = np.bitwise_and((sig_data[1::4] >> 3), 0x1f)[0:len(sig[2::3])] + 32 * np.bitwise_and(
            sig_data[3::4] >> 3, 0x1f)[0:len(sig[2::3])]
        ...
        sig[1::3] = (sig_data[1::4] >> 2)[0:len(sig[1::3])] + 64 * np.bitwise_and(sig_data[2::4], 0x0f)[
                                                                   0:len(sig[1::3])]
        # Third sample is 4 msb of third byte and 6 msb of forth byte
        sig[2::3] = (sig_data[2::4] >> 4)[0:len(sig[2::3])] + 16 * np.bitwise_and(sig_data[3::4], 0x7f)[
                                                                   0:len(sig[2::3])]
