...
class Annotation(object):
    ...
    def wrann(self, write_fs=False, write_dir=''):
        ...
        if not present_label_fields:
            raise Exception('At least one annotation label field is required to write the annotation: ',
                            ann_label_fields)
    ...
    def check_field(self, field):
        ...
        if field == 'record_name':
            ...
            for i in range(len(item)):
                ...
                if bool(re.search('[ \t\n\r\f\v]', symbol[i])):
                    raise ValueError(
                        'The symbol values of the ' + field + ' field must not contain whitespace characters')
    ...
    def get_available_label_stores(self, usefield='tryall'):
        ...
        if usefield == 'tryall':
            if self.label_store is not None:
                ...
                raise ValueError('No label fields are defined. At least one of the following is required: ',
                                 ann_label_fields)
        ...
        # Use the explicitly stated field to get available stores.
            ...
            if usefield == 'label_store':
                ...
                unused_label_stores = ann_label_table.loc[
                    ann_label_table[usefield] in unused_field, 'label_store'].values
            ...
            if self.custom_symbols is not None:
                ...
                if usefield == 'label_store':
                    ...
                    overwritten_label_stores = ann_label_table.loc[
                        ann_label_table[usefield] in overwritten_fields, 'label_store'].values
            else:
            ...
            available_label_stores = set(undefined_label_stores).union(set(unused_label_stores)).union(
...
def custom_triplet_bytes(custom_triplet):
    ...
    annbytes = [0, 88, len(custom_triplet[2]) + 3 + len(str(custom_triplet[0])), 252] + [ord(c) for c in
                                                                                         str(custom_triplet[0])] \
...
def field2bytes(field, value):
    ...
    if field == 'samptype':
        ...
        if sd > 1023:
            ...
            data_bytes = [0, 236, (sd & 16711680) >> 16, (sd & 4278190080) >> 24, sd & 255, (sd & 65280) >> 8, 0,
                          4 * typecode]
...
def proc_ann_bytes(filebytes, sampto):
    ...
    while (bpi < filebytes.shape[0] - 1):
        ...
        if sampto and sampto < sample_total:
            sample, label_store, subtype, chan, num, aux_note = rm_last(sample, label_store, subtype, chan, num,
                                                                        aux_note)
            break
...
ann_class_table = pd.DataFrame(
    {'extension': [ac.extension for ac in ann_classes], 'description': [ac.description for ac in ann_classes],
...
class AnnotationLabel(object):
    ...
    def __str__(self):
        return str(self.label_store) + ', ' + str(self.symbol) + ', ' + str(self.short_description) + ', ' + str(
            self.description)
...
ann_label_table = pd.DataFrame({'label_store': np.array([al.label_store for al in ann_labels], dtype='int'),
