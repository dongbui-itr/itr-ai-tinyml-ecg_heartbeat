...
def extract_data_artifact(data_path="/mnt/4T_DATA/LLM/strips_artifact_60s_4/",
                   output_data_path="/mnt/4T_DATA/DATA_4TINYML/tiny_hb3_data_strip_2/", #OUTPUT_PATH,
                   output_info_path="/mnt/4T_DATA/DATA_4TINYML/tiny_hb3_data_strip_2/" #OUTPUT_PATH
                          ):
    if not os.path.exists(output_data_path):
        os.mkdir(output_data_path)
    ...
    folders = os.listdir(data_path)
    fp_log = open(f"{output_data_path}/log_err_artifact.csv", "w")
    sta_beat = dict()
    sta_beat["Total"] = dict()
    """
    /mnt/4T_DATA/LLM/include-strip2//397447/67651668798462b95825c4cb/event-strip-captured-2024-12-19-08-41-40-utc-07
        ...
        for no, file in enumerate(files):
            ...
            try:
                ...
                if DEBUG:
                    ...
                    # plt.show()
                if stopSample == startSample == 0:
                    stopSample = startSample + LEN * header.fs
                ...
                while flag:
                    ...
                    output_dir = f"{output_data_path}/{eventType}/{tmp[1]}/{tmp[0]}"
                    output_dir = output_dir.replace(" ", "")
                    if not os.path.exists(output_dir):
                        os.makedirs(output_dir)
