...
def train_beat_classification(use_gpu_index,
    ...
    #     def update_state(self, y_true, y_pred, sample_weight=None):
    #         y_pred = tf.nn.softmax(y_pred, axis=-1)
    #         y_true = keras.backend.cast(y_true, self._dtype)
    #         y_pred = keras.backend.cast(y_pred, self._dtype)
    #         tf.keras.metrics.
    #         [y_true, y_pred], sample_weight = keras.metrics.  metrics_utils.ragged_assert_compatible_and_get_flat_values(
